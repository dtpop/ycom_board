...
$form = rex_config_form::factory('ycom_board');
$form->addFieldset('YCom Board - Einstellungen');

$res = rex_sql::factory()->getArray('SELECT name FROM '.rex::getTable('yform_email_template'));
$options = array_column($res, 'name');

$field = $form->addSelectField('email_template_new_thread');
$field->setLabel('E-Mail Template neues Thema');
$select = $field->getSelect();
$select->addOption('Bitte auswählen ...','');
$select->addOptions($options,true);
$field->setNotice('<code>rex_config::get("ycom_board","email_template_new_thread")</code>');

$field = $form->addCheckboxField('send_new_thread_mail');
$field->setLabel('E-Mail Benachrichtigung');
$field->addOption('E-Mails an Teilnehmer senden, wenn neues Hauptthema erstellt wird.', "1");
$field->setNotice('<code>rex_config::get("ycom_board","send_new_thread_mail")</code>');


$field = $form->addSelectField('groups_no_messages',null,['class'=>'selectpicker form-control']);
$field->setLabel('Gruppen, an die die keine Benachrichtigungen geschickt werden');
$select = $field->getSelect();
$select->addSqlOptions('SELECT name label, id value FROM '.rex::getTable('ycom_group'));
$select->setMultiple();
$field->setNotice('<code>rex_config::get("ycom_board","groups_no_messages")</code>');
